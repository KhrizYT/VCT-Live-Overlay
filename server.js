...
function publicSettingsFromUrl(url){
  ...
  const glowParam=url.searchParams.get("glow");
  const opacityRaw=opacityParam===null||opacityParam===""?NaN:Number(opacityParam);
  const glowRaw=glowParam===null||glowParam===""?NaN:Number(glowParam);
  return {
...
const server=http.createServer((req,res)=>{
  ...
  if(url.pathname==="/health")return json(res,200,{ok:true,version:"5.2.0",rooms:rooms.size,live:matches.length,providerError});
...
server.listen(PORT,"0.0.0.0",()=>{
  console.log("VLR Overlay for VCT Matches v5.2");
